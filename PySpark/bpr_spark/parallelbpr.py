...
def _optimize_partition(user_ratings, prod_mat, nb_prods, l2_reg=0.001, alpha=0.1, negative_samples=30, num_samples=20000):
...
def bpr_MF_MR(ratings, rank=10, nb_iter=10, nb_partitions=8, num_samples=100000, l2_reg=0.001, alpha=0.1,
               negative_samples=30):
    """
    Try to parallel but seems there is some problem :|
    """
    ...
    __prod_mat = np.random.uniform(size=(nb_prods + 1, rank))
