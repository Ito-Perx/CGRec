...
def _gradient_single_point(user_id, prod_id, prod_id_neg,
    ...
    x_uij = user_mat[user_id].dot(prod_mat[prod_id]) - user_mat[user_id].dot(prod_mat[prod_id_neg])

    step_size = np.exp(-x_uij) / (1 + np.exp(-x_uij)) * alpha

    user_mat[user_id] += step_size * (prod_mat[prod_id] - prod_mat[prod_id_neg]) + \
...
# Traditional BPR using spark sc format (parallel failed...)
def bprMF(ratings, rank, num_iter=10, num_neg_samples=30):
    """ 
